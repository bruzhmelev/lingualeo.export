...
lingualeo.setInterfaceLanguage = function () {
    //TODO Канго убили этот метод https://github.com/KangoExtensions/kango/issues/73
    return;
...
lingualeo.showSiteNotifications = function (notificationsArr) {
	for(var i = 0; i < notificationsArr.length; i++) {
		(function (notifInfo) {
            /*
             notification_header: "tempacc9068067509767705"
             notification_pic_url: "//d2d8jz84kf5q4n.cloudfront.net/uploads/avatar/0s50.png"
             notification_text: "Sent you a message"
             notification_url: "//lingualeo-funk.com/social/dialog/1397139159992"
             */
			var data = {
				...
				'hideTimeout': 0,
				'showAllTabs': true,
                url: 'http:' + notifInfo.notification_url + (notifInfo.notification_url.indexOf('?') === -1 ? '?' : '&') + 'utm_source=ll_plagin&utm_medium=referral&utm_campaign=notification'
			};
...
lingualeo.checkSiteNotifications = function () {
	if (lingualeo.isAuth && lingualeo.currentOptions.showBrowserPopups && lingualeo.currentOptions.showSiteNotifications) {
		...
		var currentTime = new Date().getTime();
		if (currentTime - lastCheckTime > 60 * 60 * 1000) { //if more than 12 hours
